#!/usr/bin/env python3
"""
Verifies the side a mod belongs to
Order: internal metadata (Fabric/Quilt/Forge) > Modrinth > CurseForge.
...
# ----------------------------------------------------------------------
def main():
    parser = argparse.ArgumentParser(description="Verify which side .jar Minecraft mods belong to.")
    ...
    tqdm.write(f"Summary: client={stats['client']}, server={stats['server']}, both={stats['both']}, unknown={stats['unknown']}")

    organize = input("\nWould you like to organize the mods into folders (client, server, both)? (Y/N): ").strip().lower()
    if organize == 'Y':
        base = target_dir
        ...
        tqdm.write(f"{copied} mod(s) copied to the client/, server/, and both/ folders.")
    else:
        tqdm.write("Folder organization ignored.")
